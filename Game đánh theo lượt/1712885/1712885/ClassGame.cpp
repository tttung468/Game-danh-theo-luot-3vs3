...
void ClassGame::attackListAListB() {
	...
	while(1){
		...
		//tìm cặp hero tiếp theo tấn công với nhau
		++i;
		++j;
		...
		//kiểm tra cả 2 tướng đều được đánh hay chưa
...
void ClassGame::match() {
	...
	cout << "\nround : " << round << endl;
	//xuất kết quả trận đấu
	ClassGame::resultMatch();
}

void ClassGame::testFunction() {
	cout << "test attack" << endl;
	cout << "befor attack" << endl;
	heroList[0]->outputHero();
	cout << endl;
	heroList[1]->outputHero();
	cout << endl;

	ClassGame::attackHeroAHeroB(heroList[0], heroList[1]);
